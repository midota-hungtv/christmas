function handleStartStep3() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      document.getElementById('step3').classList.add("merge");
      document.documentElement.setAttribute("data-steps", "12345");
    }, 3000);
    setTimeout(() => {
      handleActive("step4");
    ...
    */
      (function (canvas) {
        ...
        function pointOnHeart(t) {
          const scale = Math.min(canvas.width, canvas.height) * 0.35; // 👈 chỉnh 0.35–0.45
          return new Point(
            scale * Math.pow(Math.sin(t), 3),
            scale * (
              0.8 * Math.cos(t) -
              0.3 * Math.cos(2 * t) -
              0.12 * Math.cos(3 * t) -
